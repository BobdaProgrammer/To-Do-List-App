...
function CurrentCol(){
    if(Icolor == "rgb(0, 0, 0)"){
        document.getElementById("settings").style.backgroundColor = "rgb(255, 255, 255)";
    }
    else{
...
function addItem() {
    if (document.getElementById("box").value != "") {
        ...
        if(document.getElementById("box").value.includes("https://")||document.getElementById("box").value.includes("www.")){
...
function saveList() {
    ...
    localStorage.setItem("settingsCheck", document.getElementById("checkforsettings").checked.toString());
}
...
function loadList() {
    ...
    if (listHTML) {
        ...
        var importantButtons = document.querySelectorAll(".list-item .important-button");
        ...
        listItemElements.forEach(function (item) {
            ...
            var isImportant = item.querySelector(".important-button").classList.contains("active");
            if (isImportant) {
                item.querySelector(".important-button").classList.add("active");
            }
            if (settingsCheck && isFinished) {
                item.querySelector(".list-item-text").style.textDecoration = "line-through";
            }
...
function searchCheck(value,event) {
    var tasks = document.getElementsByClassName("list-item");
    if(event.key != "Backspace" && event.key != "Shift" && event.key != "ArrowUp" && event.key != "ArrowDown" && event.key != "ArrowLeft" && event.key != "ArrowRight" && event.key != "Control" && event.key != "Alt" && event.key != "CapsLock" && event.key != "Tab" && event.key != "Escape" && event.key != "AudioVolumeMute"&& event.key != "AudioVolumeUp"&& event.key != "AudioVolumeDown" && event.key != "Meta" && event.key != "Enter"){   
        value += event.key;
        console.log(value);
        if (value != ""){
        for (var i = 0; i < tasks.length; i++) {
        var taskText = tasks[i].querySelector(".list-item-text").innerHTML.toLowerCase();
...
function searchImportant(thing){
    ...
    for(var f = 0; f < listStuff.length; f++){
        var taskIColor = listStuff[f].querySelector(".list-item .important-button").style.backgroundColor;
        if(thing.style.backgroundColor == ""){
